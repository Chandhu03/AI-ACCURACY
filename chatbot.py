...
# ---- Search function ----
def search_orgs(query, province=None, type_filter=None, top_n=10):
    ...
    if type_filter:
        df_results = df_results[df_results["Type"] == type_filter]
...
so you always get credible, up-to-date data — no guesswork, no misinformation.
    ...
    selected_type = st.selectbox(
        "",
        options=["", "Government", "NGO / Private"],
...
if user_input:
    ...
    if not target_province:
        ...
        t_filter = selected_type if selected_type else None
        results = search_orgs(user_input, province=target_province, type_filter=t_filter)
